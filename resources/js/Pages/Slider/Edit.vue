    ...
    <div class="bg-white rounded-md shadow overflow-hidden">
      <form @submit.prevent="update">
        <div class="flex flex-wrap -mb-8 -mr-6 p-8">
            ...
            <img width="100px" height="40px" :src="'/'+slider.image_show" alt="Dynamic Example" />
          </div>
        <div class="flex items-center px-8 py-4 bg-gray-50 border-t border-gray-100">
          <button class="text-red-600 hover:underline" tabindex="-1" type="button" @click="destroy">Delete slider</button>
          <loading-button :loading="form.processing" class="btn-indigo ml-auto" type="submit">Update slider</loading-button>
        </div>
      </form>
    </div>
...
export default {
  ...
  methods: {
    update() {
      this.form.post(`/admin/sliders/${this.slider.id}`)
    },
  ...
  if (file) {
    ...
    destroy() {
      if (confirm('Are you sure you want to delete this slider?')) {
        this.$inertia.delete(`/admin/sliders/${this.slider.id}`)
      }
    },
  },
}
